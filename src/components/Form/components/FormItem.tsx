...
import { isArray, isString, isTrue } from '@ht/html-tool'
import { last } from 'ramda'
import { isFunctionOfOther } from '@/components/utils'
			...
			if (row.keys) {
				row.keys.forEach((res: string[]) => {
					const keysObj = {
						...
						[`onUpdate:${res[1] ?? 'value'}`]: ($event: string | ObjectMap | number) => setForName(props.model, res[0], $event),
					}
					data = {
						...data,
						...keysObj,
			...
			<Row>
				{props.rows.map((row, index) => {
					...
					return isFunctionOfOther(row.display, { record: row, dataSource: props.model, index: index }) === false ? (
						...
						<Col {...(row.colProps || { span: props.colSpan ?? 6 })} key={row.key as string}>
							{row.render ? (
