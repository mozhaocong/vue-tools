...
import { isObject, isString, isTrue } from '@ht/html-tool'
...
function setComponent(row: any, setData: any) {
	return row.render
		? row.render(setData)
		: h(row.component ? (isString(row.component) ? resolveComponent(row.component) : row.component) : resolveComponent('a-input'), {
				style: { width: '58%' },
				allowClear: true,
				...row.props,
