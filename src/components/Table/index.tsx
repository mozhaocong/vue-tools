...
import Rconfigure from '@/components/Configure'
...
const _Table = defineComponent({
	...
	setup(props, context) {
		...
		uuid++
		const unique = (componentConfig.uuid || 'r_table_unique') + '_' + uuid
		...
		return () => (
			<div class={`r_table ${unique}_r_table`}>
				<div style='position: relative;'>
					...
					<Rconfigure v-model={visible.value} onSubmit={submit} title='查询条件调整自定义' columns={clone(customRow.value) as any} />
				</div>
			</div>
		)
	},
